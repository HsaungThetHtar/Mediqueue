...
export function WaitingList() {
    ...
    // FIX #12: Removed auto-cancel based on createdAt timer — this was canceling patients
    // who booked a queue but simply hadn't arrived yet. Auto-skip is handled server-side
    // only for "in-progress" bookings (patients who were called but didn't respond in 10 min)

    // Format data for Active Waiting List — show estimated appointment time, not a bogus countdown
    const activeWaitingItems = activeItems.map(q => {
        const appointmentTime = q.estimatedTime || '—';
        return {
            id: q.id,
            queue: q.queueNumber,
            name: q.patientName,
